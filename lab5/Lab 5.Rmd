...
custom_code_checker <- function(label, user_code, solution_code, envir_result, evaluate_result, last_value, stage, ...) {
  if (stage == "code_check") {
...
custom_answer_checker <- function(label, user_code, solution_code, envir_result, evaluate_result, last_value, stage, ...) {
  if (stage == "code_check") {
      if (user_code == solution_code){
          return(list(message = random_praise(), correct = TRUE))
      }
...
gradethis_setup()
tutorial_options(exercise.completion = FALSE, exercise.checker = custom_code_checker)
```
...
10.  Now use `.predict()` to get the predicted values for the test set (save them as `preds_ols`).
...
26. To compare the models we will calculate the sum of squared errors for the from each model. Adapt the code used below that calculates the sum of squared errors for linear regression and apply it to the ridge regression and LASSO models.
