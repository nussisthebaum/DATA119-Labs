...
```{python setup1, context = "data", exercise=TRUE, exercise.eval = FALSE, message = FALSE}
import numpy as np
import pandas as pd
...
```{python setup2, context = "data", exercise = FALSE, message = FALSE, echo=FALSE, exercise.setup="setup1"}
Workinghours = Workinghours[Workinghours['hours'] > 0]
```
...
```{python setup24, context = "data", exercise = FALSE, echo=FALSE, message = FALSE, exercise.setup="setup2"}
Workinghours = pd.get_dummies(Workinghours, columns = ['occupation'], drop_first = True, dtype = float)
...
7. Take a look at the code below. You should recognize that we are first importing a function. Then, things get a bit more complicated than we are previously seen. In a single line of code, we are saving a data frame of predictors for training and testing (`X_train` and `X_test`) as well as the response variable for training and testing (`y_train` and `y_test`). You will need to do this in most data analysis problems.
...
```{python setup25, context = "data", exercise = FALSE, echo=FALSE, message = FALSE, exercise.setup="setup24"}
...
```{python setup3, context = "data", exercise = TRUE, message = FALSE, exercise.setup="setup25"}
...
```{python setup4, context = "data", exercise = TRUE, message = FALSE, exercise.setup="setup3"}
...
```{python setup5, context = "data", exercise = TRUE, message = FALSE, exercise.setup="setup4"}
...
```{python setup6, context = "data", exercise = FALSE, echo=FALSE, message = FALSE, exercise.setup="setup5"}
...
```{python setup7, context = "data", exercise = FALSE, echo=FALSE, message = FALSE, exercise.setup="setup6"}
preds_ols = model_ols.predict(X_test_pp)
```
...
```{python setup8, context = "data", exercise = FALSE, echo=FALSE, message = FALSE, exercise.setup="setup7"}
model = sklearn.linear_model.Ridge(alpha=0.5)
modelRidge = model.fit(X_train, y_train)
```

```{python setup81, context = "data", exercise = TRUE, message = FALSE, exercise.setup="setup8"}
model = sklearn.linear_model.Ridge(alpha = 0.5)
...
```{python setup9, context = "data", exercise = FALSE, echo=FALSE, message = FALSE, exercise.setup="setup81"}
import numpy as np
...
```{python cv, context = "data", exercise = TRUE, message = FALSE, exercise.setup="setup81"}
...
```{r q18, echo=FALSE}
question("18. What percent of the data is in each test fold if you perform 5-fold cross validation?.",
         answer("0%"),
         answer("10%"),
...
```{python store, context = "data", exercise = TRUE, message = FALSE, exercise.setup="cv"}
...
```{python storex, context = "data", exercise = FALSE, message = FALSE, exercise.setup="store"}
for i in range(0, len(alphas)):
  temp_model = sklearn.linear_model.RidgeCV(alphas = alphas[i]).fit(X_train_pp, y_train)
  coefs_ridge.loc[i] = temp_model.coef_
```

```{python storea, context = "data", exercise = TRUE, message = FALSE, exercise.setup="storex"}
...
```{python melt, context = "data", exercise = TRUE, message = FALSE, exercise.setup="storea"}
...
```{r q29, echo=FALSE}
question("29. Which of the variables still remain in the model at the chosen value of the hyperparameter?",
         answer("`income`"),
         answer("`age`"),
         answer("`education`"),
...
```{python melty, context = "data", exercise = FALSE, message = FALSE, exercise.setup="melt"}
model_lasso = sklearn.linear_model.LassoCV(alphas = alphas).fit(X_train_pp, y_train)
```
...
question("31. Based on the SSE from our three models, which one do you think is the best model?",
         ...
         answer("LASSO", message = "Congratulations! You found the fifth and final secret word: LANTERN.", correct=TRUE))
```