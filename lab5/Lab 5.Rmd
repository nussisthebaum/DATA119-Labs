...
```{r, sub-check, message = FALSE, warning = FALSE}
grade_this(check = function(value, ...) {
  # Get student's dataframe
  student_df <- py$Workinghours  # Access the variable from Python

  # Define the expected dataframe
  py_run_string("expected_df = Workinghours[Workinghours['hours'] > 0]")
  expected_df <- py$expected_df  # Get it in R

  # Compare dataframes
  if (py_run_string("student_df.equals(expected_df)")) {
    pass("Congratulations! You have found the first secret word: PEBBLE.")
  }
  else {
