...
1. Use `.head()` to look over the data in `arrests_raw`.
...
5. Just like with linear regression, we will need to separate our explanatory and response variables. Take this step now--name them `X_arrests` and `y_arrests.` We want to predict `released`, which tells us whether or not the arrestee was released with a summons (charged with a minor offense, as opposed to being arrested).
...
model1 = sm.Logit(y_tr, X_tr_int).fit()
```
...
output$sum_plot_output <- renderUI({
  ...
  if (isTRUE(sum_correct_poll())) {
    ...
    tags$div(
      id = "sum_hint_box",
      style = paste(
        if (!sum_hint_visible()) "display:none;" else "",
...
question("12. What is the correct interpretation on the coefficient `employed_Yes`?",
         ...
         answer("If an arrestee is not employed, the odds they are released are expected to rise by a factor of $0.7235$, all other factors held equal."),
         answer("If an arrestee is not employed, the odds they are released are expected to rise by $e^{0.7235}$, all other factors held equal."),
