...
X_gso = AirBnB_prices_raw[[...]]
X_gso = sm.add_constant(...)

gso = AirBnB_prices_raw['...']

model_gso = sm.OLS(..., ...).fit()
model_gso....
```
...
pd.get_dummies(AirBnB_prices_raw, columns = [ ... ])
```
...
AirBnB_prices = pd.get_dummies(AirBnB_prices_raw, columns = ..., dtype = ...)
```
...
X_wrong = AirBnB_prices[[ ... ]]
X_wrong = sm.add_constant(X_wrong)

model_wrong = sm.OLS(Y, ... ).fit()
model_wrong.summary()
```
...
VIF_cities = [variance_inflation_factor(..., i) for i in range(len(....columns))]
VIF_cities_df = pd.DataFrame({"feature": ....columns, "VIF": ...})
VIF_cities_df
```
...
AirBnB_prices2 = pd.get_dummies(AirBnB_prices_raw, columns = ['room_type', 'room_shared', 'room_private', 'host_is_superhost', 'city', 'dayType'], dtype = float, ...)
```
