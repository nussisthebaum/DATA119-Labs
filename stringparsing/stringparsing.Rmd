...
Recall, a **string** is a data type that represents a sequence of characters, such as letters, numbers, or punctuation. Strings in Python are written inside quotation marks — single (`' '`), double (`" "`), or triple (`''' '''` or `""" """`). For more information about creating and displaying strings, see [this textbook chapter](https://ds1.datascience.uchicago.edu/03/4/Strings.html).
...
grade_this({
  message("checker running")
  message("is.null(.result): ", is.null(.result))

  null_xptr <- tryCatch(reticulate::py_is_null_xptr(.result), error = function(e) NA)
  message("py_is_null_xptr(.result): ", null_xptr)
...
grade_this({
 ...
 if (all.equal(as.numeric(res), c(3, 7, 45)) == TRUE) {
    pass(random_praise())
  } else {
    fail(random_encouragement())
  }
