...
Now, let's one-hot-encode our categorical variables. Don't forget you need a reference level! The columns `name` and potentially even `genus` may be too granular to be of use, so we will drop those. We can also drop any rows with missing values.

```{python dummy_drop, exercise = TRUE, message = FALSE, exercise.setup = "setup-packages"}
msleep = msleep.drop(columns = ['genus','name'])
...
msleep = msleep.drop(columns = ['genus','name'])
msleep = pd.get_dummies(msleep, columns = ['vore', 'order', 'conservation'], dtype = float, dummy_NA = True, drop_first=True)
mleep = msleep.dropna()
```
...
```{python setup1, message = FALSE, warning = FALSE, echo = FALSE, exercise.setup = "setup-packages"}
msleep = msleep.drop(columns = ['genus','name'])
msleep = pd.get_dummies(msleep, columns = ['vore', 'order', 'conservation'], dtype = float, dummy_NA = True, drop_first=True)
mleep = msleep.dropna()
```

We still need to do one more thing to help us decide which variables to use - calculate the VIF. Calculate the VIF for all potential predictor variables.
...
VIF_msleep = [variance_inflation_factor(msleep.drop(columns=['sleep_total']).values, i) for i in range(len(msleep.drop(columns=['sleep_total']).columns))]
VIF_msleep_df = pd.DataFrame({"feature": msleep.drop(columns=['sleep_total']).columns, "VIF": VIF_msleep})
VIF_msleep_df
```
...
msleep = pd.get_dummies(msleep, columns = ['genus', 'order', 'conservation'], dtype = float, drop_first=True)
...
question(
  ...
  allow_retry = TRUE,
  post_message = "Congratulations! You have found the fifth and final secret word: PLANT."
)
```
