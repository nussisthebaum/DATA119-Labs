...
```{r q3, echo=FALSE}
question(
  "Is there a publisher that Professor Nussbaum seems to enjoy more than others, based on her ratings? Select all that apply.",
...
:::
The results of the previous query should seem suspicious to you. Why are there so many publishers that have perfect five star ratings?

::::

4. Write a query that answers the same question--what publisher does Professor Nussbaum enjoy the most--but considering only publishers that she have read from at least five times.
...
question(
  ...
  answer("The Hobbit And The Lord Of The Rings: Deluxe Pocket Boxed Set"), 
  answer("Spitting Gold", correct = TRUE),
...
9. Now, write a query that will result in all of the books with 0 pages. 
...
**Discuss with a neighbor (or on Ed):**
:::
Does this give you any more insight into why a book might have 0 pages?
...
SELECT title, my_rating, location, exclusive_shelf
FROM both
WHERE exclusive_shelf = "read" and location = "uk-england" AND my_rating = 2;
```
