...
grade_this({
  ...
  else fail("Hint: capture digits after `qty:` using (\\d+).")
})
```
...
grade_this({
  ...
  else fail("Hint: use a lazy capture `(.*?)` up to `(qty:`.")
})
```
...
```{r d1_names-check, message=FALSE, warning=FALSE}
grade_this({
  student <- reticulate::py_to_r(.result)
