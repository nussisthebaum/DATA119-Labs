...
```{python setup_py, context="setup", include = FALSE}
import re
...
```{python cap_demo, exercise=TRUE, message=FALSE, exercise.diagnostics = TRUE}
# Try the three patterns below and compare outputs.
# 1) no capture group -> list of full matches
no_group = re.findall(r"qty:\s*\d+", SPELLBOOK)
...
ROW_CLUES <- list(
  list(clue="Row 1: starts with A, ends with S; 4th letter is H", pat="^A..H..S$"),
  list(clue="Row 2: 7 letters; has MB in positions 3–4", pat="^..MB...$"),
  list(clue="Row 3: begins CAP; 4th letter is T or X; ends URE", pat="^CAP[TX]URE$"),
  list(clue="Row 4: begins GROUP; 6th is B or P; ends Y", pat="^GROUP[BP]Y$"),
  list(clue="Row 5: begins TOKEN; 6th is S or X; ends X", pat="^TOKEN[XS]X$"),
  list(clue="Row 6: begins FLAG; 5th is S; 6th is R or E; ends E", pat="^FLAGS[RE]E$"),
  list(clue="Row 7: begins PAR; 4th is S or T; ends ERS", pat="^PAR[ST]ERS$")
)
...
COL_CLUES <- list(
  ...
  list(
    clue="Col 6: three R's, then B, then NOT a vowel, then two R's",
    pat="^R{3}B[^AEIOU]R{2}$"
  ),
  list(
