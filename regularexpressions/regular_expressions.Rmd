...
HTML is *structured* (nested tags). That’s why scraping usually uses:

1. **An HTML parser** (reads the structure / nesting)  
2. **Regex** (for small patterns inside text, like prices or IDs)

Using regex alone is often not enough when when we need to dig through a lot of nested tags.

Today you are NOT scraping a real website. We are practicing with a small HTML snippet so you can see the idea.

Step 1: Use a parser to get structure. A very common tool for parsing HTML is the module `Beautiful Soup` which we can install using:
 `pip install beautifulsoup4` 
and import using:
 `from bs4 import BeautifulSoup`.

First, lets look at our HTML:

```{python d1_parse, exercise=TRUE, message=FALSE}
print(HTML_SNIPPET)
```

Now, we can use `BeautifulSoup()` to parse our HTML. We will use the `lxml` parser (don't worry about what this is or why we use it).
...
```{python d1_parse5, exercise=TRUE, message=FALSE}
soup.select(span)
```

Extract and print all links in our `HTML_SNIPPET`:
...
```{python d1_parse7}
# Selecting a specific element by class
main_heading = soup.select_one(".main").get_text()
print(f"Main Heading Text: {main_heading}")
