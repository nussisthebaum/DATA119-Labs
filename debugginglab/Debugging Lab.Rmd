...
tags$div(
  ...
  tags$div(
    ...
    tags$h1("Data 119 Debugging", class = "pageTitle")
  )
)
```
...
Note: The issue could be that something is missing...
...
Sometimes, the software you are using will try to warn you about syntax errors ahead of time using red or yellow lines underneath problematic parts of your code.
...
- **traceback** - the context in which the exception occurred 
    ...
    - more common when you start to write functions (tomorrow!)
...
```{python lookup3, exercise=TRUE, exercise.eval = FALSE, setup.chunk = 'lookup2'}
my_list.size
```

```{python lookup4, exercise=TRUE, exercise.eval = FALSE, setup.chunk = 'lookup2'}
my_list.mean
```
...
```{python logical2, exercise=TRUE, exercise.eval = FALSE, setup.chunk = 'logical1'}
...
```{python logical4, exercise=TRUE, exercise.eval = FALSE}
my_min(np.array([23,47,13,78]))
```
