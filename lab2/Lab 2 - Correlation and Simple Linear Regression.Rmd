...
Sys.setenv(PATH = paste(Sys.getenv("PATH"), "/opt/rstudio-connect/mnt/tmp/.local/bin", sep = ":"))

custom_checker <- function(label, user_code, solution_code, envir_result, evaluate_result, last_value, stage, ...) {
  if (stage == "code_check") {
    if (identical(user_code, solution_code)) {
      return(gradethis::random_praise())
    }
    return(gradethis::random_encouragement())
  }
  # Handle other stages explicitly (e.g., setup, initialization)
  if (stage %in% c("setup", "run")) {
    return(NULL)
  }
}
...
gradethis_setup()
tutorial_options(exercise.completion = FALSE, exercise.checker = custom_checker, theme=theme)
#tutorial_options(exercise.startup = TRUE)
```
...
tags$div(
  ...
  tags$div(
    class = "logoAndTitle",
    tags$img(
      src = "http://raw.githubusercontent.com/nussisthebaum/DATA119-Labs/refs/heads/dev_aj/lab2/img/dsi_logo.png",
      alt = "DSI Logo",
      class = "topLogo"
    ),
