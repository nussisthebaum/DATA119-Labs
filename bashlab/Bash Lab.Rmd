...
You will always be working in a specific directory on your laptop. Usually, this directory comes before either the dollar sign (`$`) or the percent sign (`%`). Check on your machine to see which one you should be looking for! On my machine, the command line looks like this:

![](images/CLI.png)

A symbol (like `$` or `%`) that lets you know that the command line is ready to receive input is known as a prompt. Once you see the prompt, you can type in a command and hit 'Enter' to execute it. An example command would be:
...
Any commands you want to use need to go after the prompt. If you execute a command like `touch newfile.txt`, it will create `newfile.txt` inside the current directory. Relative paths don't start with a slash.
...
Note that when you change a directory on your computer, you will notice that the directory listed before the prompt will change!
...
You should see a list of files, including many that you will need to solve the murder mystery. 

Please note that due to how our server is set up, you will have to change the directory to `data` at the beginning of each chunk. Now, show the `instructions` file. 
...
Eventually you might want to be able to run Python from the terminal! To do so, simply use the command `python`:

```{bash example42, echo = TRUE, eval = FALSE}
python
```

Using this command results in something like

```{bash example43, echo = TRUE, eval = FALSE}
(base) amynussbaum@Amys-MacBook-Pro ~ % python
...
<!-- ![](images/CLI_python.png) -->

Note that once you are running Python, the prompt changes. 

```{r q4, echo=FALSE}
question(
  "According to the screenshot, what is the prompt for Python?",
  answer("`$`",),
  answer("`%`"),
  answer("`>>>`", correct = TRUE),
  answer("`help`"),
...
You can also run a single line of Python code using the `-c` option! Note that you will have to use the `print()` command, as well as quotation marks, to actually return the results of any calculations. Edit the following line of code to use Python to find the sum of 8 and 7. 

```{bash ex8, exercise = TRUE}
python -c "print(2+2)"
```

```{bash ex8-solution, message = FALSE, warning = FALSE, echo = FALSE}
python -c "print(8+7)" 
```
...
Several single lines of code listed one right after the other can be saved as a Python script. You may be used to working only with notebooks so far, but a document containing only Python commands (no plain text or explanations) usually has a `.py` extension. To run the script, you just type the name after `python`, like:

```{bash example44, echo = TRUE, eval = FALSE}
python example_script.py
```

This will execute everything inside `example_script.py` linearly (each command after the one before it). Note that you either have to be working inside the directory where the script is saved (check with `pwd`) or change to the directory before you try to run it. 

You will also want to install Python packages and modules from the command line. You can do this using `pip`, which stands for `P`ip `I`nstalls `P`ackages, or `P`ip `I`nstalls `P`ython, depending on who you ask. `pip` is a program typically included with most Python installations. Once you specify that you would like to use `pip`, you also need to include a command--in this case, `install`. Finally, you need the name of the package. For example, say you want to install the package `numpy`:

```{bash example45, echo = TRUE, eval = FALSE}
pip install numpy
```

Please note that if you use Anaconda, `pip` will not work for you! Instead, use `conda`:

```{bash example46, echo = TRUE, eval = FALSE}
conda install numpy
```

If you are not sure which to use, try `pip` first--don't be afraid to test it out, the worst that will happen is an error message, and then you can move on to `conda`.

There are also other `pip` commands, like `uninstall`, `inspect`, `list`, `show`, and `check`. Use the [`pip` documentation](https://pip.pypa.io/en/stable/cli/) to answer the following questions?

```{r q5, echo=FALSE}
question(
  "Which `pip` command will tell you which Python packages are installed?",
  answer("`uninstall`",),
  answer("`inspect`"),
...
```{r q6, echo=FALSE}
question(
  "Which `pip` command will verify that the installed packages have compatible dependencies?",
  answer("`uninstall`",),
  answer("`inspect`"),
  answer("`list`"),
  answer("`show`"),
...
For quick references, check out:

* [Command Line Murder Mystery](https://github.com/veltman/clmystery)
* [Command Line Cheatsheet](https://github.com/veltman/clmystery/blob/master/cheatsheet.pdf)
* [`pip` Documentation](https://pip.pypa.io/en/stable/cli/)

In addition, if you find a resource that you think is very helpful, let us know!
