...
* Learn how to write your own custom error messages. 
...
Consider another form of communication: writing an academic paper. Typically, academic papers can be broken down into parts: 
...
$$\text{profit} = \text{price} \times (\text{attendees}) - (180 + 0.04\times\text{attendees})$$
...
* Functions begin with a special Python keyword, `def`, as in `def`ine. Functions must have names--similar to how you would name variables in Python code. In the code above, my function name is `function_name`. The [style guide](https://uchicago-cs.github.io/student-resource-guide/style-guide/python.html) also states we should try to have a verb in the name, so it is a better description of what the function actually does.
* Your function name should follow [snake case](https://en.wikipedia.org/wiki/Snake_case), just like variables. 
* The function name preferably includes a verb to demonstrate what it actually does!
...
If the function is meant to to calculate something based on the arguments and provide it back to the function that calls it, then use the keyword `return`. 
...
Note that the [style guide](https://uchicago-cs.github.io/student-resource-guide/style-guide/python.html) also states that we should be using doc strings. The functions in this lab do not have doc strings, but make sure to review the syntax of a doc string and know how to write a good description of what the function does. This is a one time pass, the rest of your work should have them!

When should you use a function? Look for:

* Things that belong together, 
* Things that have names, and 
* Rhings you might want to do a lot.

For example, we have already considered that we might want to calculate the number of attendees as a function of `price`. Fill in the right equation below so that we have a new function, `attendees()`.
...
Note that when you run this code, nothing happens--or at least, it looks like nothing is happening. Although there is no output, `calc_attendees()` has been defined as a function and now exists in your environment, even though Python did not print anything. That's okay! 
...
Anything that is not a valid month year PAIR constitutes an error! And based on the way we have written a function, we wouldn't know--it will return `31` for any entry. 
...
We are used to being frustrated when we encounter errors. Errors are not necessarily a bad thing, as they sometimes give us information about how we are wrong. For more descriptions of what each of these means, you may want to look at our [Debugging Lab](https://posit.ds.uchicago.edu/debugging/). For now, we will focus on writing our own errors when we are writing our own functions! Remember that our code looks like:
