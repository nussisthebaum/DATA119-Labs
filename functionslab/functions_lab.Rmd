...
def cost(price):
    return 180 + 0.04*attendees(price)
...
As a capitalist, the movie theater owner wants to know: **Can we make more by changing the price?** Define a discount as the difference between the base price, `5`, and the lower price attendees are actually paying. From previous experience, assume we know that for every ten cents we lower the price, an additional 15 people will come. 
 
 
$$\text{price}*(120 + 15 * (5 - \text{price})/0.1) - 180 - 0.04*\text{attendees}$$
...
Functions can use other functions! In the chunk below, edit the code for the function `cost()` so that it depends on `attendees()`.
...
## Remember that attendees() depends on price, and therefore, that cost() should also depend on price.
...
Now, write a function for `revenue()` using the code below. 
...
Finally, write a function `profit()` in the chunk below. 
...
Why don't I have to write `is_leap_year(year) == True`? 1Using the implicit `True` is a tiny bit faster, and using the implicit `False` adheres to the style guide. Overall there is much less visual clutter!
