...
`np.append` takes in an array **arr** and adds the new **values** to the end of a copy of it and stores the combination of the elements from **arr** and **values** in a new array.
...
```{python q8, exercise = TRUE, message = FALSE, exercise.setup="setup_py"}
coin = ['Heads', 'Tails']
num_iterations = ...
results_array = ...
for i in np.arange(num_iterations):
...
add a discussion question about how this could be done without explicitly creating or updating any arrays (counter approach)?

add q9 where we'll calculate the actual probability
