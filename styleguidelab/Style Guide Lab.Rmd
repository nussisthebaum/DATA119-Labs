...
data = pd.read_csv(".www/nussbaum_goodreads_library_export.csv", index_col = "Title")
```
...
Who write style guides? Usually, organizations that want to encourage collaborating between different programmers, including large companies, like Google, or the University of Chicago Computer Science Department (both of which are similar and contain principles we will be using in class. )
...
epi_mini = read_csv"(https://github.com/nussisthebaum/DATA119-Labs/blob/main/datawranglinglab/data/epi_mini.csv")
```
...
* **3a. Two modules named `y` are to be imported.** This should not happen frequently, but we can't avoid two independent developers coming up with the same name for different modules. In particular, this may help with what are called "aliased" functions--functions that have the same name. 
...
* **3b. `y` conflicts with a top-level name defined in the current module.** This means that you are attempting to use the name `y` for a new import, variable, or function, but the name `y` is already in use at the top level of your current file or module. The best way to fix it is to give the new function an alias so you can use both.
...
1. Comprehensions, a common way to create new sequences from existing sequences, and generator expressions, which create iterators, should be short! They are okay to use for simple cases, but multiple `for` clauses or filter expressions are not permitted. Optimize for readability, not conciseness. For example, consider the following list comprehension, named `result`:
...
Use the "implicit" `True`/`False` if at all possible (with a few caveats), e.g., `if foo:` rather than `if foo != []:`. 
...
Thankfully, there are tools to help us adhere to this particular style guide convention! Use this guide to help [turn on Ruler in VSCode](https://uchicago-cs.github.io/student-resource-guide/vscode/config.html). This should give you a visual signifier when you exceed the line length.
...
question(
  ...
  answer("NUMBER_OF_STUDENTS", correct = TRUE),
  allow_retry = TRUE
)
```
