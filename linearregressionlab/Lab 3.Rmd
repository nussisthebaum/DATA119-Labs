...
# To download this file go to https://posit.ds.uchicago.edu/linearregressionlab/www/AirBnB_mini.csv
...
question_checkbox(
  HTML("
    3. Which issues are present in the residual plot shown?<br><br>
...
```{r q4, echo = FALSE}
question_checkbox(
  HTML("4. Which issues are present in this residual plot shown?<br><br>
...
5. Using `statsmodels` syntax, refit a simple linear regression model predicting `realSum` from `dist`, and make a note of the $R^2$ (which can be found in the `.summary()` output.

```{python slr, exercise = TRUE, message = FALSE, paged.print = FALSE}
X1 = AirBnB_prices[...]
X1 = sm.add_constant(X1)
...
```{r slr_reformat, context="server"}
slr_correct_poll <- reactivePoll(500, session,
  checkFunc = function() get_tutorial_state()$slr$correct,
  valueFunc = function() get_tutorial_state()$slr$correct
)

slr_hint_visible <- reactiveVal(FALSE)

output$slr_plot_output <- renderUI({
  print(get_tutorial_state())
  if (isTRUE(slr_correct_poll())) {
    tagList(
    ...
    renderImage({
      filename <-normalizePath(file.path("./images/q1_reformat.png"))
    ...
    tags$div(
      id = "slr_hint_box",
      style = paste(
        if (!slr_hint_visible()) "display:none;" else "",
    ...
    tagList(
      p("For some reason the formatting of the `summary()` on tutorials hosted on our server is very bad. Answer Question 1 correctly to reveal reformatted results.")
    )
  }
}
)
...
```{r slr_conditional, echo=FALSE, eval=TRUE, message = FALSE, warning = FALSE}
uiOutput("slr_plot_output")
```


6. You might remember that the `dir()` function can identify different objects within another object, and that in `statsmodels` objects, there are lot of useful, pre-calculated objects. Apply it to `model1`.
...
```{r q7, echo=FALSE}
question("7. Which object do you think contains the predicted values for this model?",
...
```{r q8, echo=FALSE}
question("8. Which object do you think contains the residuals for this model?",
...
```{python dp_extract, exercise = TRUE, message = FALSE}
...
```{python log_diag, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
Y2 = AirBnB_prices["logRealSum"]
...
```{python m2, exercise=TRUE, message = FALSE}
model2.___
```
...
```{r m2_reformat, context="server"}
m2_correct_poll <- reactivePoll(500, session,
  checkFunc = function() get_tutorial_state()$m2$correct,
  valueFunc = function() get_tutorial_state()$m2$correct
)

m2_hint_visible <- reactiveVal(FALSE)

output$m2_plot_output <- renderUI({
  print(get_tutorial_state())
  if (isTRUE(m2_correct_poll())) {
    tagList(
    p("For some reason the formatting of the `summary()` on tutorials hosted on our server is very bad. Take a look at how it should appear in a Jupyter notebook or other IDE."),
    renderImage({
      filename <-normalizePath(file.path("./images/q16_reformat.png"))
      list(src = filename, width = "450px")
    },  deleteFile = FALSE),
    #actionButton("m2_show_hint_btn", "💡 Show hint"),
    tags$div(
      id = "slr_hint_box",
      style = paste(
        if (!slr_hint_visible()) "display:none;" else "",
    ...
    tagList(
      p("For some reason the formatting of the `summary()` on tutorials hosted on our server is very bad. Answer Question 16 correctly to reveal reformatted results.")
    )
  }
}
)
...
```{r m2_conditional, echo=FALSE, eval=TRUE, message = FALSE, warning = FALSE}
uiOutput("m2_plot_output")
```
...
```{python setup4, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
X3 = AirBnB_prices[['person_capacity', 'cleanliness_rating', 'guest_satisfaction_overall', 'bedrooms', 'dist', 'metro_dist', 'attr_index_norm', 'rest_index_norm', 'lng', 'lat']]
X3 = sm.add_constant(X3)
...
```{python setup4-solution, message = FALSE, warning = FALSE, echo = FALSE}
X3 = AirBnB_prices[['person_capacity', 'cleanliness_rating', 'guest_satisfaction_overall', 'bedrooms', 'dist', 'metro_dist', 'attr_index_norm', 'rest_index_norm', 'lng', 'lat']]
X3 = sm.add_constant(X3)
...
```{r setup4-code-check, message = FALSE, warning = FALSE}
grade_this_code()
```

```{r setup4_reformat, context="server"}
setup4_correct_poll <- reactivePoll(500, session,
  checkFunc = function() get_tutorial_state()$setup4$correct,
  valueFunc = function() get_tutorial_state()$setup4$correct
)

setup4_hint_visible <- reactiveVal(FALSE)

output$setup4_plot_output <- renderUI({
  print(get_tutorial_state())
  if (isTRUE(setup4_correct_poll())) {
    tagList(
    p("For some reason the formatting of the `summary()` on tutorials hosted on our server is very bad. Take a look at how it should appear in a Jupyter notebook or other IDE."),
    renderImage({
      filename <-normalizePath(file.path("./images/q20_reformat.png"))
      list(src = filename, width = "450px")
    },  deleteFile = FALSE),
    #actionButton("setup4_show_hint_btn", "💡 Show hint"),
    tags$div(
      id = "setup4_hint_box",
      style = paste(
        if (!setup4_hint_visible()) "display:none;" else "",
        "margin-top:.6rem; border:1px solid #ddd; padding:.6rem; border-radius:.5rem;"
      ),
      "")
    )
...
# observeEvent(input$setup4_show_hint_btn, {
#   shinyjs::toggle(id = "setup4_hint_box", anim = TRUE, time = 0.2)
#   setup4_hint_visible(!setup4_hint_visible())
#   updateActionButton(
#     session, "setup4_show_hint_btn",
#     label = if (setup4_hint_visible()) "🙈 Hide hint" else "💡 Show hint"
#   )
# })
```

```{r setup4_conditional, echo=FALSE, eval=TRUE, message = FALSE, warning = FALSE}
uiOutput("setup4_plot_output")
```
...
```{python diagplots, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
model3_df = model3.fittedvalues.to_frame(name = 'Fitted')
model3_df['Residuals'] = model3.resid
...
```{python setup5, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
model4 = sm.OLS(Y2, X3).fit()
model4.summary()
```

```{python setup5-solution, message = FALSE, warning = FALSE, echo = FALSE}
model4 = sm.OLS(Y2, X3).fit()
model4.summary()
```

```{r setup5-code-check, message = FALSE, warning = FALSE}
grade_this_code()
```

```{r setup5_reformat, context="server"}
setup5_correct_poll <- reactivePoll(500, session,
  checkFunc = function() get_tutorial_state()$setup5$correct,
  valueFunc = function() get_tutorial_state()$setup5$correct
)

setup5_hint_visible <- reactiveVal(FALSE)

output$setup5_plot_output <- renderUI({
  print(get_tutorial_state())
  if (isTRUE(setup5_correct_poll())) {
    tagList(
    p("For some reason the formatting of the `summary()` on tutorials hosted on our server is very bad. Take a look at how it should appear in a Jupyter notebook or other IDE."),
    renderImage({
      filename <-normalizePath(file.path("./images/q23_reformat.png"))
      list(src = filename, width = "450px")
    },  deleteFile = FALSE),
    #actionButton("setup5_show_hint_btn", "💡 Show hint"),
    tags$div(
      id = "setup5_hint_box",
      style = paste(
        if (!setup5_hint_visible()) "display:none;" else "",
        "margin-top:.6rem; border:1px solid #ddd; padding:.6rem; border-radius:.5rem;"
      ),
      "")
    )
...
# observeEvent(input$setup5_show_hint_btn, {
#   shinyjs::toggle(id = "setup5_hint_box", anim = TRUE, time = 0.2)
#   setup5_hint_visible(!setup5_hint_visible())
#   updateActionButton(
#     session, "setup5_show_hint_btn",
#     label = if (setup5_hint_visible()) "🙈 Hide hint" else "💡 Show hint"
#   )
# })
```

```{r setup5_conditional, echo=FALSE, eval=TRUE, message = FALSE, warning = FALSE}
uiOutput("setup5_plot_output")
```
...
```{python diagnostic_plots_log, exercise=TRUE, message = FALSE}
...
<!-- ```{python mlrsummary, exercise=TRUE, exercise.eval = FALSE, message = FALSE} -->
<!-- model4.summary() -->
<!-- ``` -->

![](./images/q23_reformat.png){width=80%}
...
```{r q28, echo=FALSE}
question("28. If you were carrying out backward model selection, the first variable you would remove is... ",
         answer("`const`", message = "We almost never remove the intercept, focus on the slopes instead!"),
         answer("`person_capacity`"),
         answer("`cleanliness_rating`"),
         answer("`guest_satisfaction_overall`"),
         answer("`bedrooms`"),
         answer("`dist`"),
         answer("`metro_dist`", correct = TRUE),
...
```{r q29, echo=FALSE}
question("29. True or False: If you were to remove a variable from this model and refit, the coefficients on the remaining variables would remain the same.",
         answer("True"),
         answer("False", correct = TRUE),
...
output$residual_bucket_ui <- renderUI({
  bucket_list(
    header = "1. Drag each residual plot to the issue it illustrates:",
...
output$outlier_bucket_ui <- renderUI({
  bucket_list(
    header = "2. Drag each residual plot to the type of outlier it shows:",
    group_name = "outliers",
    orientation = "horizontal",
    add_rank_list(
      text = "Plots:",
