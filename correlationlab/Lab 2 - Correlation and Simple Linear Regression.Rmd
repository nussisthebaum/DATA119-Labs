...
# go to https://posit.ds.uchicago.edu/correlationlab/www/<name of file here>.csv
...
```{python slopec, exercise = TRUE, message = FALSE}
r_fatcal = 
s_fat = 
s_cal = 
...
```{python int, exercise = TRUE, message = FALSE}
def calc_int(r_xy, s_x, s_y, m_x, m_y):
    b1 = 
    b0 = 
    return(b0)
...
```{python intc, exercise = TRUE, message = FALSE}
m_fat = 
m_cal = 
...
```{python predc, exercise = TRUE, message = FALSE}
fat = epicurious_nd['fat']
cal = epicurious_nd['calories']
...
```{python res, exercise = TRUE, message = FALSE}
def calc_res(x, y):  
    preds = 
    resids = 
    return(resids)
...
```{python resc, exercise = TRUE, message = FALSE}
pred_res
```
...
```{python sse, exercise=TRUE, exercise.eval = FALSE}
def calc_SSE(x, y):  
    resids = calc_res(x, y)
    resids2 = resids**2
...
```{python scipy, exercise=TRUE, exercise.eval = FALSE}
import scipy
from scipy.optimize import minimize
```
...
```{python sseo, exercise = TRUE, message = FALSE}
def calc_SSE(parms, x, y):  
    preds = 
    resids = 
    resids2 = 
...
```{python minset, exercise = FALSE, echo=FALSE, message = FALSE}
def calc_SSE(parms, x, y):  
    preds = parms[0] + parms[1]*x
    resids = y - preds
    resids2 = resids**2
...
```{python min, exercise = TRUE, message = FALSE}
...
```{python stats, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
import statsmodels.api as sm
```
...
```{python stats1, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
X1 = epicurious_nd["fat"]
Y = epicurious_nd["calories"]
```
...
```{python stats2, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
X1 = sm.add_constant(X1)
```
...
```{python stats3, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
model1 = sm.OLS(Y, X1).fit()
```
...
```{python summary, exercise=TRUE, exercise.eval = FALSE, message = FALSE}
#| output: asis
#| results: asis
...
If you were to write up a hypothesis test to see if `fat` is a significant predictor of `calories` to include in a data analysis report...

```{r q26, echo=FALSE}
question(
  "26. What parameter would your hypotheses be about?",
  answer("$\\beta_0$, the intercept in a model predicting calories from fat.", message = "Sometimes we run a hypothesis test on the intercept, but usually we are far more interested in the slope."), 
  answer("$\\beta_1$, the slope in a model predicting calories from fat.", correct = TRUE),
  answer("$b_0$, the estimated intercept in a model predicting calories from fat.", message = "Don't forget! Hypotheses should be about parameters, not statistics."),
answer("$b_1$, the estimated slope in a model predicting calories from fat.", message = "Don't forget! Hypotheses should be about parameters, not statistics."),
  random_answer_order = TRUE,
  allow_retry = TRUE
)
```
...
```{r q27, echo=FALSE}
question(
  "27. Which of the following is an appropriate pair of hypotheses?",
  answer("$H_0: \\beta_1 \\geq 0$ vs. $H_A: \\beta_1 < 0$", message = "Sometimes we run a one-sided test, but usually we are far more interested in a two-sided test."), 
  answer("$H_0: \\beta_1 = 0$ vs. $H_A: \\beta_1 \\neq 0$", correct = TRUE), 
  answer("$H_0: \\beta_1 \\leq 0$ vs. $H_A: \\beta_1 > 0$", message = "Sometimes we run a one-sided test, but usually we are far more interested in a two-sided test."),
  allow_retry = TRUE)
```

```{r q28, echo=FALSE}
question(
  "28. Based on the output, what is the test statistic?",
  answer("12.7294", message = "That is slope!"), 
  answer("0.341", message = "That is standard error for the slope!"), 
  answer("8.701", message = "That is the test statistic for the intercept!"), 
  answer("37.347", correct = TRUE), 
  answer("0.000", message = "That is the p-value for the slope!"), 
  random_answer_order = TRUE,
  allow_retry = TRUE
)
```
...
```{r q29, echo=FALSE}
question(
  "29. Based on the output, what is the test statistic?",
  answer("12.7294", message = "That is slope!"), 
  answer("0.341", message = "That is standard error for the slope!"), 
  answer("8.701", message = "That is the test statistic for the intercept!"), 
  answer("37.347", message = "That is the test statistic for the slope!"), 
  answer("0.000", correct = TRUE), 
  random_answer_order = TRUE,
  allow_retry = TRUE
)
```
...
```{r q30, echo=FALSE}
question(
  "30. Based on this p-value, what would you do with the null hypothesis?",
...
1. How would you draw a conclusion to this test, based on the context of this problem?
2. Do you have any additional context that might shed light on how reasonable this value is? Is there another value you would like to test?
