...
correct_answers_resid <- list(
  good = HTML('<img src="./images/good_fit_residual_plot.png" height="150">'),
  nonlinear = HTML('<img src="./images/non-linearity_residual_plot.png" height="150">'),
  hetero = HTML('<img src="./images/heteroscedasticity_residual_plot_updated.png" height="150">'),
  non_normal = HTML('<img src="./images/non-normal_residuals_residual_plot.png" height="150">')
)
...
output$residual_bucket_ui <- renderUI({
  bucket_list(
    ...
    add_rank_list(
      text = "Plots:",
      labels = unname(correct_answers_resid),
...
observeEvent(input$submit_resid, {
  good_input <- input$good %||% list()
  nonlinear_input <- input$nonlinear %||% list()
  hetero_input <- input$hetero %||% list()
  non_normal_input <- input$non_normal %||% list()

  # Convert all values to plain strings for comparison
  result <- all(
    identical(as.character(good_input), as.character(list(as.character(correct_answers_resid$good)))),
    identical(as.character(nonlinear_input), as.character(list(as.character(correct_answers_resid$nonlinear)))),
    identical(as.character(hetero_input), as.character(list(as.character(correct_answers_resid$hetero)))),
    identical(as.character(non_normal_input), as.character(list(as.character(correct_answers_resid$non_normal))))
  )
