...
```{r q5, echo = FALSE}
question(
  "5. Match each regression diagnostic pattern to the issue it represents:",
  type = "matching",
  list(
    "Good Fit (no violations)" = "good",
    "Non-Linearity" = "nonlinear",
    "Heteroscedasticity" = "hetero",
    "Non-Normal Residuals" = "non_normal"
  ),
  list(
    "good" = '<img src="good_fit_residual_plot.png" height="100">',
    "nonlinear" = '<img src="non-linearity_residual_plot.png" height="100">',
    "hetero" = '<img src="heteroscedasticity_residual_plot.png" height="100">',
    "non_normal" = '<img src="non-normal_residuals_residual_plot.png" height="100">'
  ),
  correct = TRUE,
  allow_retry = TRUE,
  random_answer_order = TRUE
...
```{r q6, echo = FALSE}
question(
  "Match the type of outlier to its corresponding plot:",
  type = "matching",
  list(
    "High Leverage Points" = "leverage",
    "Large Residuals" = "residuals",
    "High Leverage + Large Residuals" = "both",
    "Clustered Outliers" = "cluster"
  ),
  list(
    "leverage" = '<img src="high_leverage_points_residual_plot.png" height="100">',
    "residuals" = '<img src="large_residuals_residual_plot.png" height="100">',
    "both" = '<img src="both_high_leverage_&_large_residuals_residual_plot.png" height="100">',
    "cluster" = '<img src="clustered_outliers_residual_plot.png" height="100">'
  ),
  correct = TRUE,
  allow_retry = TRUE,
  random_answer_order = TRUE
...
```{r q7, echo = FALSE}
question(
  "7. Which issues are present in the residual plot shown?",
  type = "multiple",
  ...
  img = "non_normal_large_outliers_residual_plot.png",
  allow_retry = TRUE,
  random_answer_order = TRUE
)
```
...
```{r q8, echo = FALSE}
question_checkbox(
  "8. Which issues are present in this residual plot?",
  ...
  img = "exponential_combo_residual_plot.png",
  allow_retry = TRUE,
  random_answer_order = TRUE
)
...
11. Sometimes when the response variable is particularly skewed, we can easily address failed assumptions by applying a mathematical transformation to the different variables. Money-related distributions are almost always skewed, but this one is particularly bad in part due to a few very extreme values (specifically, a 1,854,545 euro per night one bedroom home in Athens). 
...
12. Now, edit this cell to fit a new model predicting `logRealSum` from `dist`. Make a note of the $R^2$. Examine the diagnostic plots from the new model, which you should call `model2`.
...
```{r q10, echo=FALSE}
question("10. True or False: A linear model is more appropriate for these two variables than the previous variables.",
...
13. The $R^2$ for both models is fairly low, so we already have some idea of whether `dist` is a helpful predictor. Let's practice formally finding significant features in the model.
...
14. Print the summary output from `model2`.
...
```{r q15, echo=FALSE}
question("15. Would you reject or fail to reject a formal hypothesis test investigating whether or not the coefficient is zero?",
         answer("Reject"),
...
16. `dist` may be significant, but as we can see from the adjusted $R$-squared, it doesn't predict `logRealSum` very well. Let's try adding more variables! Run this cell to take a look at `AirBnB_prices` and use it to answer the following question--again, check out the [Kaggle documentation](https://www.kaggle.com/datasets/thedevastator/airbnb-prices-in-european-cities) if you need.
...
17. To confirm assumptions, edit the code so that it will calculate the correlation matrix for the numeric variables (we can ignore indicators for Lab 3). For easier reading, I've rounded to three decimals. 
...
18. Now let's try multiple linear regression. Run the cell below to fit a model predicting `realSum` with the numeric variables--be careful to use only one set of the `attr` and `rest` variables.
...
19. Run this cell to look at the diagnostic plots of this model.
...
20.  Don't forget that the transformed response had much better diagnostic plots for the simple linear regression! Run the cell below to predict `logRealSum`.
...
```{r q20, echo=FALSE}
question("20. True or False: A linear model is more appropriate for this data set.",
         answer("True", correct = TRUE),
         answer("False"),
         allow_retry = TRUE,
...
```{r q22, echo=FALSE}
question_checkbox("22. Which of the variables are significant at the 5% level? Select ALL that apply.",
...
```{r q23, echo=FALSE}
question("23. For every one bedroom increase, log price is expected to  ",
