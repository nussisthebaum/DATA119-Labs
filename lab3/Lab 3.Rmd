...
6. Now, using `model1_df`, edit the cell to produce the diagnostic plots (a scatterplot of residuals against the predicted (aka fitted) values, and a histogram of the residuals).
...
18.  Don't forget that the transformed response had much better diagnostic plots for the simple linear regression! Run the cell below to predict `logRealSum`.
...
19. Redo the diagnostic plots (use the code above as a reference if you need!).
...
```{r q21, echo=FALSE}
question_checkbox("21. Which of the variables are significant at the 5% level? Select ALL that apply.",
...
```{r q22, echo=FALSE}
question("22. For every one bedroom increase, log price is expected to  ",
